<template>
  <div class="schedule-page">
    <div class="page-header">
      <div>
        <h2>课程安排</h2>
        <p class="page-desc">管理您的课程排班时间表</p>
      </div>
      <el-button type="primary" @click="showSetSchedule" class="add-btn" :disabled="!selectedDate">
    ...
    <div class="empty-state" v-else>
      <el-empty :description="selectedDate ? '该日期暂无排班，请设置' : '请选择日期查看排班'">
        <template #image>
...
import { getMySchedule, setMySchedule } from '@/api/schedule'
...
const loadSchedule = async () => {
  ...
  try {
    const res = await getMySchedule(selectedDate.value)
...
const handleSaveSchedule = async () => {
  ...
  try {
    const res = await setMySchedule(selectedDate.value, tempSchedules.value)
    if (res.data.code === 200) {
